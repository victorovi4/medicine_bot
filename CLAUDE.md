...
Персональная электронная медицинская карта пациента Иоффе В.Б. Построена на Next.js 16 (App Router), TypeScript, Prisma, деплоится на Vercel. Весь код находится в подкаталоге `medical-card/`. Интерфейс и медицинский контент — на русском языке.

## Команды

Все команды запускаются из `medical-card/`:
...
- **БД**: Vercel Postgres через Prisma 6 ORM (10 моделей в `prisma/schema.prisma`)
- **Хранилище файлов**: Vercel Blob
- **AI**: Claude Sonnet 4 через OpenRouter API (не Anthropic SDK напрямую)
...
**Пайплайн AI-анализа** (`src/lib/claude.ts`): Файл → OpenRouter API (Claude Vision для изображений, извлечение текста для PDF) → структурированный JSON → автозаполнение полей документа, извлечение измерений, детекция процедур (гемотрансфузии, операции).

**Telegram-бот** (`src/app/api/telegram/webhook/route.ts`): Принимает фото/PDF, запускает AI-анализ, сохраняет в БД. Поддерживает пакетный режим (много фото → один PDF через pdf-lib) и обнаружение дубликатов с подтверждением через inline-кнопки.
...
**Система метрик** (`src/lib/metrics.ts`): Настроенные метрики (ПСА, Гемоглобин, СРБ) с алиасами для нечёткого сопоставления, нормальными диапазонами и автоизвлечением из документов. Визуализация через Recharts в `src/components/MetricsChart.tsx`.
...
### Деплой
Vercel с корневой директорией `medical-card/`. Скрипт `vercel-build` выполняет `prisma generate && prisma db push --skip-generate && next build`. Таймаут функций — 60с для `/api/analyze` и `/api/telegram/webhook` (настроено в `vercel.json`).
