...
5. **Динамика состояния** (dynamics):
   Как менялись ключевые показатели (гемоглобин, ПСА, СОЭ и т.д.).
   
   КРИТИЧЕСКИ ВАЖНО — проверяй математику:
   - Если значение УВЕЛИЧИЛОСЬ (например, 84 → 107), пиши "повысился" или "увеличился"
   - Если значение УМЕНЬШИЛОСЬ (например, 107 → 84), пиши "снизился" или "уменьшился"
   - НЕ ПУТАЙ направление изменения! Всегда сравнивай числа.
   
   Для гемоглобина: если с 84 до 107 — это ПОВЫШЕНИЕ (положительная динамика).
   Для СОЭ: если с 61 до 120 — это ПОВЫШЕНИЕ (отрицательная динамика, т.к. СОЭ должен быть низким).
